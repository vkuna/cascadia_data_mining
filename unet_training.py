...
epos=20 # how many epocs?
std=0.2 # how long do you want the gaussian STD to be?
subset=1 # train on a subset or the full monty?
...
if ponly:
    ...
    if subset:
        model_save_file="./unet_10000_pn_eps_"+str(epos)+"_std_"+str(std)+".tf" 
        n_data=n_data[:10000,:]
        p_data=p_data[:10000,:]
    else:
        model_save_file="./unet_250000_pn_eps_"+str(epos)+"_std_"+str(std)+".tf"
else:
    n_data, _ = pickle.load( open( 'pnsn_ncedc_N_training_data.pkl', 'rb' ) )
    s_data, _ = pickle.load( open( 'pnsn_ncedc_S_training_data.pkl', 'rb' ) )  
    if subset:
        model_save_file="./unet_10000_sn_eps_"+str(epos)+"_std_"+str(std)+".tf" 
        n_data=n_data[:10000,:]
        s_data=s_data[:10000,:]
    else:
        model_save_file="./unet_250000_sn_eps_"+str(epos)+"_std_"+str(std)+".tf"
...
checkpoint_filepath = './checks/'+model_save_file[2:]
